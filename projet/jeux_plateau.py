...
class JeuxPlateau:
    ...
    def run(self):
        ...
        if pion_selectionne:
            ...
            self.deplacement = Deplacement(pion_choisi=pion_instance, de=self.de, cases_reculer=self.map.cases_reculer, cases_map = self.map.cases_map, cases_question = self.map.cases_question)
        else:
            print("Aucun pion n'a été sélectionné. Fin du jeu.")
            return False

        clock = pygame.time.Clock()  # Pour gérer le temps et la fluidité
        running = True
        while running:
